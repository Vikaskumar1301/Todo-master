...
# Task
1. Determine the base branch: prefer `main`, else `master`, else remote HEAD.
2. Run `git diff <base>...HEAD --name-only` to get changed files.
3. Examine each changed file against the checklist below.
4. Output findings and a final verdict.

# Checklist — evaluate every changed file

## Architecture Compliance
- ✅/❌ Routes only call Services — no direct repository calls from routes?
- ✅/❌ Services only call Repositories — no SQLAlchemy imports in services (except type hints)?
...
- ✅/❌ Pydantic schemas used for all request/response DTOs (no raw dicts)?

## Security
- ✅/❌ No hardcoded secrets, passwords, or connection strings in any file?
- ✅/❌ All user input validated via Pydantic schemas before reaching repository?
...
- ✅/❌ No blocking I/O (file reads, network calls) inside async functions without await?
...
- ✅/❌ Non-null new columns have `server_default` or staged migration?

## Test Coverage
- ✅/❌ Every new public function has at least one test?
- ✅/❌ Edge cases covered (empty input, not found, duplicate, invalid)?
- ✅/❌ Tests don't call real external services (DB mocked in unit tests)?

## Code Quality
- ✅/❌ No `print()` in Python — `logging` module used?
- ✅/❌ No `console.log()` left in TypeScript (unless intentional debug with comment)?
- ✅/❌ No commented-out code?
- ✅/❌ Google-style docstrings on all new public Python functions?
- ✅/❌ TSDoc comments on complex TypeScript functions?
- ✅/❌ No abbreviations in variable/function names (`tmp`, `x`, `d`)?
...
- ⚠️ **REQUEST CHANGES** — Issues found. Fix before merging. (list critical items)
- 💬 **NEEDS DISCUSSION** — Design questions need resolution before proceeding.
